...
Елси мы скачали чей-то **Fork** со своего аккаунта для внесения изменений с целью сделать последующий `pull request` - нам желательно производить все изменения в новой ветке, для этого можно воспользоваться следующей командой (после создания локальной копии): `git checkout -b <NewBranchName>` 

#### Скачивание данных в локальный репозиторий

Чтобы загрузить и слить последние изменения из удаленного репозитория необходимо использовать команду `git pull`

#### Закачивание данных в удалённый репоизторий

Чтобы cкачать и слить последние изменения в удалённый репозиторий необходимо использовать команду `git push`

**ВНИМАНИЕ !!!** Если локально создана новая ветвь то для её первоначальной загрузки необходимо использовать комманду `git push --set-upstream origin <NewBranchName>` 
